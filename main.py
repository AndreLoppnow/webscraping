...
def scrape_produtos_detalhes(produto_url, produto_categoria):
    """
    Coleta os detalahes de um produto apartir do link.
    """    
    print(f"Buscando detalhes do produto e informações técnicas do protudo: {protuto_url}")
    
    respsonse = requests.get(produto_url)
    if respsonse.status_code != 200:
        print(f"Erro ao acessar a página: {respsonse.status_code}")
        return None
    
    soup = BeautifulSoup(respsonse.content, 'html.parser')
